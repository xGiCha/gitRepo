...
class RepoViewModel @ViewModelInject constructor(
    ...
    /** ROOM DATABASE */
//    val readCommits: LiveData<List<ReposEntity>> = repository.local.readCommits().asLiveData()

    fun readCommits(id: String): LiveData<List<ReposEntity>> {
        return repository.local.readCommits(id).asLiveData()
    }


    fun insertCommits(reposEntity: ReposEntity) =
            viewModelScope.launch(Dispatchers.IO) {
                repository.local.insertCommits(reposEntity)
            }
